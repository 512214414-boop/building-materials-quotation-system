// 当前单据行表头级联：锁条件 + 从已加载行里收窄。
// 选项本身走后端 GET /documents/:id/lines/facets（同一套锁语义）。
// 与 backend/src/services/documentLineFacets.ts 保持同步。
...
export interface DocumentLineFacetOption {
  type: 'existing';
  label: string;
  value: string;
  id: string;
