...
> ⚠️ **方向作废（2026-09-05）**：用户反思"标准 3NF 已满足业务目标，产品名不需要独立字典"，遂将 v23 ①③（产品名升全局字典 + 分类改关系表）**整体回退**——P1/P2a/P2b 代码已外科手术式摘除，P4 未落库故无需执行。下方论证仅作历史留存，不再作为实施依据。
...
**D. 迁移路（分阶段）**
- P0 文档（本步）：v23 实施口径写入台账待审。 → ✅ 已审（用户裁决启动）→ **2026-09-05 方向作废**
- P1 schema/meta/生成（**不动 DB**）：→ ✅ 完成后于 **2026-09-05 回退**（product_name/product_category 模型 + productNameId 字段 + meta 三处均摘除，saveProduct 经 git revert 还原；全量 typecheck 复绿）
- P2 后端改写：→ ✅ 完成后于 **2026-09-05 回退**（P2a product_name 字典基础设施 / P2b saveProduct 全局唯一键均撤销）
- P3 前端改写：→ ❌ 未做即随方向作废
- P4 DB 执行：→ ✅ **无需**（方向已回退，未落库）
- P5 验收：→ ❌ 无需
...
| **档案类页面统一化·Phase 5 删旧壳（2026-09-05）** | product 编辑弹窗从 `renderDialog` 逃生口迁为标准档案槽：品牌/规格走 `cascade` 槽（options/onSelect/addCell/editRow 全经 `ctx.extras`）；单位+价格走 `custom` 槽复用 `UnitSection`、图片走 `custom` 槽复用 `BrandImages`、分类走 `custom` 槽复用 `ArchiveDialogField`、名称/俗称走 `name`/`scalar` 槽；瞬态态从组件 useState 收口到 `ctx.extras`，保存前异步确认（分类解析+规格重复+空品牌+自动补默认确认）走新增通用钩子 `beforeSave`、装配走 `collectPayload`。**删除 `ProductEditDialog.tsx`（1300 行旧壳，无外部引用）**；框架新增 `ArchiveBeforeSaveCtx.beforeSave` + `ArchiveDialogCtx.message/modalConfirm`（通用能力，非 product 后门）；`create`/`update`/`seed`/`loadSeed`/`validate`/`afterSave` 接 `saveProduct`。验证：build + `tsc -p tsconfig.app.json` + lint + dupe + cell-layer + 后端单测全绿；snowflake 单测本环境 5s 默认超时偏紧（30s 下 1.59s 通过），非逻辑回归。⚠️ 产品编辑是唯一核心高频流程，**行为需真机冒烟**（品牌切换重载/规格切换/单位基准归一/价格矩阵保存/图片上传）确认零回归后再标完全交付 |
