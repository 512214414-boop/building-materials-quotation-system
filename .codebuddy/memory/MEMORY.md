...
- **文档之家（2026-09-05 裁决 A）**：文档可视化＝全仓人读文档的内容之家——方法论 truth source 在 `data-source/methodology/**`，**系统决策/设计文档迁入 `项目文档/**`**（架构蓝图 / 根因分析与预防措施 / 档案类统一化设计 已从仓库根迁入）；仓库根只留工具加载层（AGENTS / docs-coverage / 开发规划 / 掌控台等生成物）。归属判据已沉淀进方法论 know-layout「文档家族归属」表（四类：工具加载层/方法论条目/系统决策/业务归档），AGENTS 顶注与 gen-boss-view/check-arch 引用路径已同步。
