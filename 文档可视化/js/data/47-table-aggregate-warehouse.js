...
DOC_VIZ.tableAggregateWarehouse = {
  ...
  guardActions: ["inventory_adjust", "inventory_opening", "inbound_set_target"],

  // 出发点 / 要支持到：集合体视角自写（不复制旧 warehouse-model 的五段档案正文）
  intent: {
    kicker: "出发点 · 货从哪出",
    title: "配货要立刻知道从内部仓还是外部渠道走",
    lead: "开完单下一问是货从哪出。店里有 → 内部仓；店里没有 → 刚问到的渠道。两棵树。现场正文在侧栏「指导思想 · 配货履约」，本页只落结论。",
    rules: [
      ["店里有货", "从哪一仓哪一区位出 → 配货来源 · 内部仓库列表"],
      ["店里没有", "找外部渠道 → 配货来源 · 外部供应商列表"],
      ["内部仓不是供应商", "自有库存 vs 外部渠道 → warehouse 与 supplier 永久拆开"]
    ]
  },

  need: {
    kicker: "要支持到 · 场景推出层级",
    title: "主档 + 两张 N：区位 / 负责人都是子表",
    lead: "库房需求简单但有两个容易错的地方：区位无默认（与负责人相反）、inventory 五键只读独立成台账页。层级由这些现场事实推出。",
    rules: [
      ["一仓多个区位", "A 区 / B 区 / 待检 → 区位子表 N（无默认，showDefault 关闭）"],
      ["一仓多个负责人", "联系矩阵 → 负责人子表 N（默认互斥）"],
      ["配货要选从哪出", "内部仓或供应商 → 开单配货来源 · 两棵树"],
      ["库存按仓看", "warehouse × zone × spec × brand × unit 五键 → 独立台账页，不进档案弹窗"]
    ]
  },
