...
DOC_VIZ.orderFramework = {
  ...
  viewForm: {
    kicker: "视图形态 · 不是层级",
    title: "八个视图是同一张单的八种投影",
    lead: "视图不是新表，是同一批数据按筛选 / 分组 / 汇总重切的一个切面。所以它不进结构树——树里没有它的节点。加一个视图不等于加一张表。",
    rules: [
      ["投影", "同一主表按条件重切：筛选 / 分组 / 汇总。数据没多一份，只是看法不同。"],
      ["不是", "不是新表、不是新层级、结构树里不设节点。"],
      ["明细才是结构", "明细表才是单据主表的子层（树里有节点）。视图不落在结构这一维。"],
      ["判据", "问一句：加了它会不会多一张表？不会 → 视图形态；会 → 明细 / 档案。"],
      ["界面", "视图沿用点值表同一套壳（ViewFrame + 行表 + 槽位），不套 ArchiveListPage。"]
    ]
  },
  implOrder: {
    kicker: "落地顺序",
